...
通过保留完整上下文 + 尾部追加目标内容，LLM 在生成旧内容的摘要时，**能看到之后发生的对话**。这意味着：

- **早期被修正的结论**，在摘要中能体现最终的正确状态
- **矛盾的消息**可以自然消解——后续发展说明哪个方向是最终采用的
- **对话连续性**不受影响，原文保留在原位置，尾部追加的副本仅为 LLM 参考用

LLM 读完完整上下文后，生成的摘要自然包含了之后的发展。
