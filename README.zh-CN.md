...
│ user: [node0_001] 第0-69条: <摘要>       │  ← 前文已压缩的摘要
│ user: [node1_001] 第70-139条: <摘要>     │
│ user: 原始消息 140                        │
│ user: 原始消息 141                        │  ← 待压缩段（保留在
│ ...                                      │     原有位置不动）
│ user: 原始消息 209                        │
│ user: 原始消息 210                        │
│ user: 原始消息 211                        │  ← 后续消息（待压缩
│ ...                                      │     段之后的内容）
│ user: 原始消息 279                        │
│ user: [COMPRESS] 压缩以下对话...          │  ← 压缩指令
│ user: 原始消息 140                        │
│ user: 原始消息 141                        │  ← 待压缩段（复制
│ ...                                      │     一份到尾部）
│ user: 原始消息 209                        │
│ user: Summary:                           │  ← 引导输出
└─────────────────────────────────────────┘
...
传统压缩只看到被压缩的片段，不知道后面发生了什么。通过保留完整上下文 + 尾部追加目标内容，LLM 在生成旧内容的摘要时，**能看到之后发生的对话**，这解决了两个常见问题：

- **减少错误累积**：如果早期的结论在后面被修正了，摘要可以体现最终的正确状态
- **上下文冲突感知**：两条矛盾的对话存在时，后续发展能说明哪个方向才是最终采用的

待压缩的原文同时出现在两个位置：原位置（保持对话连续性）和尾部（带上压缩指令）。LLM 读完完整上下文后，生成的摘要自然包含了之后的发展。
